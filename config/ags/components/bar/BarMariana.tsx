...
// -----------------Vars---------------------
export const time = Variable("").poll(countMinutes(1), ["bash", "-c", `date +"%-l:%M %P"`])
export const dateAll = Variable("").poll(countHours(10), ["bash", "-c", `date +"%a %b %d"`])
...
function Clock() {
    return <button className="clock-btn" cursor="pointer">
        <box>
...
function createButton(text: string, onClickCommand: string) {
    return (
...
function MenuShortcuts() {
    return (
        <box className="menu-shortcuts">
            {createButton("Screenshot", "~/.config/ags/launch.sh screenshot")}
            {createButton("Picker", "hyprpicker -f hex | cat | wl-copy")}
            {createButton("Keyboard", "whoami")}
        </box>
    );
}
function AppLauncher() {
    return <button className="app-launcher-btn" halign={Gtk.Align.START} cursor="pointer" onClicked={
        () => {
            safeExecAsync(["bash", "-c", "~/.config/rofi/launcher/launch.sh"])
        }
    } >
        <icon icon="redhat-symbolic" />
    </button>
}
function MediaBar() {
    return <box className="media">
        ...
        <label label=" · " />
        <label label={bind(artist)} maxWidthChars={16} truncate={true} />
        <icon
...
function Menu() {
    return <button className="menu-btn" cursor="pointer" onClicked={
        () => {
...
export default function Mariana(monitor: Gdk.Monitor) {
    if (!monitor) {
    ...
    >
        ...
        <centerbox>
            <box hexpand halign={START}>
                <AppLauncher />
                <label className="app-label" halign={Gtk.Align.START} label={bind(nameWindowHypr)} />
                <MenuShortcuts />
                <Hack />
                <MediaFocus />
            </box>
